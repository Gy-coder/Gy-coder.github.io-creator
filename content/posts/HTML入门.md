...
在 Vscode 中创建 html 文件后，在英文状态下输入!后按 tab 键就会生成 html 的基本结构
...
4. id:
      ...
      - id 在 HTML 中不会报错，但是 id 有重复时，js 会报错

      - js 可以直接获取 id

      - window.后的内容作为 id 名的话，js 无法获取
...
6. tabindex:
     ...
     指示其元素是否可以聚焦，以及它是否/在何处参与顺序键盘导航（通常使用 Tab 键，因此得名）。

     tabindex=负值 (通常是 tabindex=“-1”)表示不能通过键盘导航(Tab)来访问到该元素
